...
class Command(BaseCommand):
    ...
    def _clear_test_data(self):
        ...
        deleted_avail = LeaderAvailability.objects.filter(notes__icontains="[TESTE]").delete()[0]

        # Remove músicos de teste (pelo username)
        test_usernames = [m["username"] for m in self.TEST_MUSICIANS]
        deleted_musicians = Musician.objects.filter(user__username__in=test_usernames).delete()[0]

        deleted_users = User.objects.filter(username__in=test_usernames).delete()[0]
    ...
    def _create_musicians(self, org):
        ...
        for data in self.TEST_MUSICIANS:
            ...
            self.stdout.write(
                f"  - {data['first_name']} {data['last_name']} ({data['instrument']}) [{status}]"
            )
    ...
    def _print_summary(self, musicians):
        ...
        total_public = LeaderAvailability.objects.filter(
            notes__icontains="[TESTE]", is_public=True
        ).count()
        total_private = LeaderAvailability.objects.filter(
            notes__icontains="[TESTE]", is_public=False
        ).count()
