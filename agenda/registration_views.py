        ...
        if isinstance(instruments_raw, list):
            ...
            if len(instruments_raw) > 10:
                ...
                for item in instruments_raw:
                    ...
                    if len(value) > 50:
                        errors["instruments"] = (
                            "Cada instrumento deve ter no máximo 50 caracteres."
                        )
                        break
...
class CheckEmailView(APIView):
    ...
    def get(self, request):
        ...
        if not email:
            return Response(
                {"error": "Email não fornecido."}, status=status.HTTP_400_BAD_REQUEST
            )
...
class RegistrationStatusView(APIView):
    ...
    def get(self, request):
        ...
        if not pending:
            return Response(
                {"error": "Cadastro não encontrado."}, status=status.HTTP_404_NOT_FOUND
            )
...
class ResendVerificationView(APIView):
    ...
    def post(self, request):
        ...
        return Response({"message": "Email de verificação reenviado!"})
...
class SubscriptionActivateFakeView(APIView):
    ...
    def post(self, request):
        ...
        if org:
            org.subscription_status = "active"
            org.save()
            Membership.objects.filter(user=request.user, organization=org).update(
                status="active"
            )
...
class PaymentCallbackView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "Invalid payment token"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            with transaction.atomic():
                ...
                if payment_method in ["pix", "boleto"]:
                    days = 30 if plan == "monthly" else 365
                    musician.subscription_ends_at = timezone.now() + timedelta(
                        days=days
                    )
                else:
                    musician.subscription_ends_at = None
...
class SubscriptionStatusUpdateView(APIView):
    ...
    def post(self, request):
        ...
        logger.info(
            f"Subscription status updated for musician {musician.id}: {new_status}"
        )
