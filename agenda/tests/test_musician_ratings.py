...
from agenda.models import Availability, Event, Membership, Musician, MusicianRating, Organization
...
class MusicianRatingAPITest(APITestCase):
    ...
    def setUp(self):
        ...
        self.creator = User.objects.create_user(
            username="arthur", email="arthur@test.com", first_name="Arthur", password="senha123"
        )
        ...
        self.creator_musician = Musician.objects.create(
            user=self.creator, instrument="guitar", role="member", organization=self.org
        )
        self.musician1 = Musician.objects.create(
            user=self.musician1_user, instrument="vocal", role="member", organization=self.org
        )
        self.musician2 = Musician.objects.create(
            user=self.musician2_user, instrument="drums", role="member", organization=self.org
        )
        ...
        Availability.objects.create(
            musician=self.musician1, event=self.past_event, response="available"
        )
        Availability.objects.create(
            musician=self.musician2, event=self.past_event, response="available"
        )
    ...
    def test_submit_ratings_fails_for_future_event(self):
        ...
        response = self.client.post(
            f"/api/events/{self.future_event.id}/submit_ratings/", payload, format="json"
        )
    ...
    def test_rating_updates_musician_average(self):
        ...
        Musician.objects.create(
            user=other_creator, instrument="bass", role="member", organization=self.org
        )
        ...
        Availability.objects.create(
            musician=self.musician1, event=other_event, response="available"
        )
