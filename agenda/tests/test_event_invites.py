...
class EventInviteFlowTest(APITestCase):
    ...
    def setUp(self):
        ...
        Membership.objects.create(
            user=self.creator, organization=self.org, role="owner", status="active"
        )
        ...
        Membership.objects.create(
            user=self.user3, organization=self.org, role="member", status="active"
        )
        self.musician3 = Musician.objects.create(
            user=self.user3, instrument="bass", role="member", organization=self.org, is_active=True
        )
