...
class ConnectionAndBadgeAPITest(APITestCase):
    def setUp(self):
        ...
        self.musician = Musician.objects.create(user=self.user, instrument='guitar', organization=self.org)
        self.target = Musician.objects.create(user=self.target_user, instrument='vocal', organization=self.org)
