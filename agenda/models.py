...
class Musician(models.Model):
    ...
    stripe_subscription_id = models.CharField(
        max_length=100, blank=True, null=True, help_text="Stripe Subscription ID"
    )
    ...
    subscription_ends_at = models.DateTimeField(
        blank=True, null=True, help_text="Data de término da assinatura"
    )

    # Trial fields
    trial_ends_at = models.DateTimeField(
        blank=True, null=True, help_text="Data de término do período de teste"
    )
    trial_started_at = models.DateTimeField(
        blank=True, null=True, help_text="Data de início do período de teste"
    )
...
class PendingRegistration(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default="pending_email"
    )
    email_token = models.CharField(
        max_length=64, unique=True, help_text="Token de verificação de email"
    )
    ...
    expires_at = models.DateTimeField(
        help_text="Data de expiração do cadastro pendente"
    )
    ...
    def complete_registration(self):
        ...
        if not instruments and self.instrument:
