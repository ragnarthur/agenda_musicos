            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 mb-6 transition-colors duration-200">
              <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                ...
                <span className="ml-auto text-xs font-normal text-gray-400">
                  {musician.portfolio_videos.length} vídeo
                  {musician.portfolio_videos.length > 1 ? 's' : ''}
                </span>
              </h2>
              <div
              ...
              >
                {musician.portfolio_videos.map((video, index) => (
                  <div key={`${video.provider}-${video.video_id}`}>
                    <div className="aspect-video rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-900">
                      <iframe
                        src={video.embed_url}
                        title={`Vídeo ${index + 1} de ${musician.full_name}`}
