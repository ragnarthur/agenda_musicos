...
const compareByImportance = (
  ...
  const scoreDiff =
    getEventImportanceScore(b, currentUserId) - getEventImportanceScore(a, currentUserId);
...
const EventsList: React.FC = () => {
  ...
  const statistics = useMemo(() => {
    const confirmed = filteredEvents.filter(
      ev => ev.status === 'confirmed' || ev.status === 'approved'
    );
    return {
  ...
  const renderEventCard = useCallback(
    (event: Event) => {
      ...
      const canManage = Boolean(
        currentUserId && event.created_by === currentUserId && event.status !== 'cancelled'
      );
      const destructiveType =
        event.status === 'confirmed' || event.status === 'approved' ? 'cancel' : 'delete';
              ...
              <div>
                ...
                <div className="mt-6 flex flex-wrap gap-3">
                  <div className="rounded-2xl border border-indigo-200/70 bg-gradient-to-br from-indigo-50/80 via-white/90 to-white/70 px-4 py-3 text-sm font-semibold text-gray-800 shadow-lg backdrop-blur dark:border-indigo-800/60 dark:from-indigo-900/30 dark:via-slate-900/70 dark:to-slate-900/60">
                    ...
                    <p className="text-2xl text-indigo-700 dark:text-indigo-200">
                      {statistics.total}
                    </p>
                  </div>
          ...
          <div className="rounded-2xl border border-white/60 bg-white/90 p-4 sm:p-5 shadow-xl backdrop-blur">
            ...
            <div className="mt-5 flex gap-2 overflow-x-auto pb-2 -mx-4 px-4 no-scrollbar sm:flex-wrap sm:overflow-visible sm:pb-0 sm:mx-0 sm:px-0">
              ...
              >
                <span className="relative inline-flex h-2.5 w-2.5">
                  <span
                    className={`absolute inline-flex h-full w-full rounded-full ${
                      pendingResponsesCount > 0
                        ? 'animate-ping bg-amber-400 opacity-60'
                ...
                <Zap
                  className={`h-4 w-4 ${showPendingResponses ? 'text-white' : 'text-amber-600'}`}
                />
            ...
            <div className="space-y-4">
              {groups.map(group => {
                return (
                  ...
                  >
                    <div className="rounded-[18px] border border-white/70 dark:border-white/10 p-4 shadow-lg backdrop-blur bg-white/80 dark:bg-slate-900/40">
