...
const DustParticles3D: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const makeCluster = () => ({
      cx: area.x + area.w * (0.65 + Math.random() * 0.3),
      baseY: area.y + area.h * (0.25 + Math.random() * 0.5),
      ...
      for (let i = 0; i < CLUSTER_COUNT; i += 1) {
        for (let j = 0; j < PARTICLES_PER_CLUSTER; j += 1) {
          particles.push({
            ...
            spin: (Math.random() * 0.6 + 0.4) * (Math.random() < 0.5 ? -1 : 1),
    ...
    const render = (time: number) => {
      ...
      clusters.forEach((cluster) => {
        cluster.cx -= cluster.speed * delta;
        const wave = Math.sin(time * 0.0006 + cluster.phase);
        const sway = Math.cos(time * 0.0004 + cluster.phase * 1.3);
        const clusterY = cluster.baseY + wave * (area.h * 0.06) + sway * (area.h * 0.03);

        if (cluster.cx < area.x + area.w * 0.1) {
          cluster.cx = area.x + area.w * (0.92 + Math.random() * 0.06);
          cluster.baseY = area.y + area.h * (0.25 + Math.random() * 0.5);
        }

        cluster.baseY = clusterY;
      });

      particles.forEach((particle) => {
        const cluster = clusters[particle.clusterIndex];
        particle.angle += particle.spin * delta * (1 + particle.z * 0.8);

        const radius = particle.radius * (0.7 + particle.z * 0.6);
        const offsetX = Math.cos(particle.angle + time * 0.0002) * radius;
        const offsetY = Math.sin(particle.angle + time * 0.0003) * radius * 0.8;
