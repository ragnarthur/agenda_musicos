...
import { mockMusicians, mockEvents, mockAuthResponse, mockInstruments, mockConnections, mockBadges, mockGigs, } from './data';
...
export const handlers = [
    ...
    http.get(`${API_URL}/musicians/:id/connections/`, ({ params }) => {
        const musicianConnections = mockConnections.filter(c => c.follower.id === Number(params.id) || c.target.id === Number(params.id));
    ...
    http.get(`${API_URL}/events/`, ({ request }) => {
        ...
        if (search) {
            ...
            filteredEvents = filteredEvents.filter(e => e.title.toLowerCase().includes(searchLower) ||
        ...
        if (pendingResponses === 'true' || pendingResponses === '1') {
            filteredEvents = filteredEvents.filter(e => e.created_by === 1 && ((e.availability_summary?.pending || 0) > 0));
        }
        if (past === 'true') {
    ...
    http.get(`${API_URL}/events/pending_my_response/`, () => {
        const pendingEvents = mockEvents.filter(e => e.availabilities?.some(a => a.response === 'pending'));
    ...
    http.get(`${API_URL}/instruments/`, ({ request }) => {
        ...
        if (q) {
            const filtered = mockInstruments.filter(i => i.display_name.toLowerCase().includes(q.toLowerCase()) ||
